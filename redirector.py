...
def main():
	...
	while request:
		...
		if (time_change != int(getmtime(FILENAME))):
			redirect_urls = json.load(open(FILENAME))
			time_change = int(getmtime(FILENAME))
			logging.debug(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ": FILE CHANGE" )
		else:
			logging.debug(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ": FILE not CHANGE {}".format(time_change))
		...
		for url in redirect_urls.keys():
			if url in request[0]:
				...
				break 
		if response:
			logging.debug(datetime.now().strftime('%Y-%m-%d %H:%M:%S') + 'CHANGE: ' + response +'\n')
